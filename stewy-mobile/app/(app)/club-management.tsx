...
import { volunteerService, type VolunteerProfileResponse } from '@/services/volunteer.service';
...
export default function ClubManagementScreen() {
  ...
  const [pendingMembers, setPendingMembers] = useState<VolunteerProfileResponse[]>([]);
    ...
    try {
      ...
      if (p.club) {
        ...
        const [membersData, pendingData] = await Promise.all([
          ...
          api.get<VolunteerProfileResponse[]>(`/clubs/${p.club.id}/members/pending`),
        ]);
  ...
  const handleApproveMember = async (volunteerId: string) => {
    if (!profile?.club) return;
    try {
      await api.post(`/clubs/${profile.club.id}/members/${volunteerId}/approve`, {});
  ...
  const handleRejectMember = async (volunteerId: string) => {
    if (!profile?.club) return;
    try {
      await api.post(`/clubs/${profile.club.id}/members/${volunteerId}/reject`, {});
      setPendingMembers((prev) => prev.filter((m) => m.id !== volunteerId));
    } catch (err: any) {
      Alert.alert('Error', err.message);
    }
  ...
  return (
    <SafeAreaView className="flex-1" edges={['top']}>
      <View className="px-6 pt-4 pb-2 flex-row items-center justify-between">
        ...
        <ThemedButton label="Back" variant="muted" size="sm" onPress={() => router.back()} />
      </View>
      ...
      >
        ...
        {profile?.club && (
          <ThemedCard className="mb-6 p-4">
            <ThemedText type="h3" className="mb-4">Club Info</ThemedText>
            <ThemedInput
              label="Club Name"
              value={clubName}
        ...
        {/* Member Roster */}
        <ThemedCard className="mb-6 p-4">
          <ThemedText type="h3" className="mb-4">Member Roster ({members.length})</ThemedText>
          {members.length === 0 ? (
            ...
            members.map((member) => (
              <View key={member.id} className="flex-row items-center justify-between py-2 border-b border-gray-100 dark:border-gray-800">
                <View>
                  <ThemedText type="bodySemiBold">{member.firstName} {member.lastName}</ThemedText>
                  <ThemedText type="caption" muted>{member.role}</ThemedText>
                </View>
        ...
        {/* Pending Requests */}
        <ThemedCard className="mb-6 p-4">
          <ThemedText type="h3" className="mb-4">Pending Requests ({pendingMembers.length})</ThemedText>
          {pendingMembers.length === 0 ? (
            ...
            pendingMembers.map((member) => (
              <View key={member.id} className="py-3 border-b border-gray-100 dark:border-gray-800">
