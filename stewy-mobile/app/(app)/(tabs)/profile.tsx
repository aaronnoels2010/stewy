...
export default function ProfileScreen() {
  ...
  const profileBadgeVariant = profile?.profileStatus === 'APPROVED' ? 'success' : 'warning';
  ...
  const steps = [
    ...
    { key: 'approved', label: t('pendingGate.steps.approved'), done: profile?.profileStatus === 'APPROVED' },
    { key: 'game', label: t('pendingGate.steps.game'), done: false },
  ];

  const isHoofdSteward = profile?.role === 'HOOFD_STEWARD' || isAdmin;

  return (
    <SafeAreaView className="flex-1" edges={['top']}>
      <ScrollView className="flex-1 px-6 pt-4">
        <ThemedText type="h1" className="mb-6">{t('profile.title')}</ThemedText>
        ...
        <ThemedCard className="mb-6 p-4">
          {steps.map((step, index) => (
            <View key={step.key} className="flex-row items-center mb-2">
              ...
              >
                {step.done && <ThemedText type="bodySmall" className="text-white font-bold">✓</ThemedText>}
              </View>
              <ThemedText
                type="bodySmall"
        ...
        {user && (
          <ThemedCard className="mb-6 p-4">
            <ThemedText type="h3" className="mb-4">{t('profile.userInfo')}</ThemedText>
            ...
            {user.phone && <ThemedText type="bodySmall" variant="muted">{user.phone}</ThemedText>}
            {user.address && <ThemedText type="bodySmall" variant="muted">{user.address}</ThemedText>}
          </ThemedCard>
        )}
        ...
        {isLoading ? (
          ...
          <ThemedCard className="mb-6 p-4">
            <View className="flex-row justify-between items-center mb-4">
              ...
              <ThemedBadge variant={profileBadgeVariant} label={profile.profileStatus} dot />
            </View>
            ...
            {profile.club && (
              <View className="flex-row items-center gap-2 mt-1">
                ...
                {profile.clubStatus && (
                  <ThemedBadge variant={clubStatusBadge(profile.clubStatus)} label={profile.clubStatus} dot />
                )}
              </View>
            )}
        ...
        {/* Club Management (HoofdSteward only) */}
        {isHoofdSteward && (
          <ThemedButton
            label="Club Management"
            variant="outline"
            onPress={() => router.push('/(app)/club-management' as any)}
