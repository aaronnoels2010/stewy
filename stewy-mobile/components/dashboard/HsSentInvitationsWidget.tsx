...
export function HsSentInvitationsWidget() {
  ...
  useEffect(() => {
    participationService.getPendingInvitationsForClub()
      .then(setInvitations)
  ...
  return (
    <BaseWidget
      title={t('dashboard.widget.sentInvitations')}
      count={invitations.length}
      emptyState={t('dashboard.empty.sentInvitations')}
      loading={loading}
      error={error ?? undefined}
      viewAllLink="/(app)/(tabs)/games"
      viewAllLabel={t('dashboard.viewAll', { count: invitations.length, type: t('dashboard.widget.sentInvitations').toLowerCase() })}
    >
      {displayed.map((inv) => (
        <TouchableOpacity key={inv.volunteerId} onPress={() => router.push('/(app)/(tabs)/games')}>
